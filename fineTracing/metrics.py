...
def calculate_iou(true_mask, gen_mask) -> float:  # TODO: add threshold
...
def pair_countours(true_contours, gen_contours) -> list:
    """Pair generated contours with their corresponding ground truth contours."""
    paired_cracks = []
...
def main() -> None:
    ...
    for file in os.listdir(ground_frames_dir):
        ...
        gen_frame_path = os.path.join(
            gen_frames_dir, f"{frame_number}.{config.output_format}"
        )
        output_overlayed_path = os.path.join(
            output_overlay_dir, f"{frame_number}.{config.output_format}"
        )
        ...
        # --------------- Calculate metrics for each crack in the frame ---------------------
        it_metrics = {}
